...
def train(file_pattern, eval_dir, model_dir, metrics_inter, 
    ...
    # init model
    noise = torch.randn(conf.batch_size, conf.noise_dim)
    generator = Generator(
    ...
    out_disc = discriminator(
        torch.ones(conf.batch_size, 3, conf.img_size, conf.img_size)
    )
    print(discriminator)
    print('D Output shape:', out_disc.shape)
    ...
    gan.build(
        g_optimizer=torch.optim.Adam(
            ...
            lr=conf.g_lr, 
            betas=(conf.g_beta1, conf.g_beta2)
        ),
        d_optimizer=torch.optim.Adam(
            discriminator.parameters(),
            lr=conf.d_lr, 
            betas=(conf.d_beta1, conf.d_beta2)
        ),
    ...
    noise_seed = torch.randn(num_examples_to_generate, conf.noise_dim)
